...
<div class="ms_cont_wrap clearfix">
  ...
  <tbody>
  <c:choose>
   <c:when test="${clist ne null && clist.size() != 0 }">
   <c:forEach items="${clist }" var="covo">
    <tr>
      <td class="orList">${covo.cname }</td>
      <td class="orList">${covo.cprice }</td>
      <td class="orList">${covo.camount }</td>
      <td class="orList">${covo.cpay }</td>
      <td class="orList">${covo.cbuyday }</td>
    </tr>
   </c:forEach>
   </c:when>
   <c:otherwise>
    <tr>
      <td colspan="5" style="text-align: center;">음료 주문내역이 없습니다.</td>
    </tr>
    </c:otherwise>
    </c:choose>
   </tbody>
   ...
   <div style="height: 60px;">
      <div class="page_board">
        <ul class="pagination">
          <c:if test="${pgvo.prev }">
            <li>
            <a class="page-link" href="/member/history?pageNum=${pgvo.firstPage - 1 }&amount=${pgvo.cri.amount}&email=${mInfo.email }"><i class='fas fa-angle-left' style='font-size:15px'></i></a>
            </li>
          </c:if>
          <c:forEach begin="${pgvo.firstPage }" end="${pgvo.lastPage }" var="i">
            <li>
            <a class="page-item ${pgvo.cri.pageNum == i ?'active':'' }" href="/member/history?pageNum=${i }&amount=${pgvo.cri.amount }&email=${mInfo.email }">${i }</a>
            </li>
          </c:forEach>
          <c:if test="${pgvo.next }">
            <li>
            <a class="page-link" href="/member/history?pageNum=${pgvo.lastPage + 1 }&amount=${pgvo.cri.amount}&email=${mInfo.email }"><i class='fas fa-angle-right' style='font-size:15px'></i></a>
            </li>
          </c:if>
        </ul>
      </div>
