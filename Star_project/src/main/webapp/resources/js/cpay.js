function transferToModal(cname,cprice,camount){
	$(document).find(".modal-title").text(cname);
...
function starCard(res,email) {
   ...
   trTag += '</select><p id="gnum"></p><p>잔액 : <span id="gprice"></span>원</p>'
      ...
      +'<tr><th colspan="2"><a href="#" id="payBt" class="tpayBtn">주문하기</a></th></tr>';
   $("#chPay").after(trTag);
   let selVal = $("#sel option:selected").val();
   getCinfo(email, selVal,totalCprice);
}

function getCinfo(email, selVal) {
   let totalPrice = $("#totalCprice").text();
   console.log("totalCprice: "+totalPrice);
...
function paysubmit() {
    ...
    $("#payForm").attr("action","/corder/other");
